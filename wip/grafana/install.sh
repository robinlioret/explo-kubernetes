...
kubectl create secret generic grafana-admin-credentials --namespace monitoring --from-literal=GF_SECURITY_ADMIN_USER=admin --from-literal=GF_SECURITY_ADMIN_PASSWORD=azerty12345
kubectl apply --server-side --namespace monitoring -f ./grafana/resources.yaml
